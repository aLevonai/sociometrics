...
export default async function AdminEmployeesPage() {
  ...
  const { data: employees } = await supabase
    .from('employees')
    .select('id, military_id, full_name, email, role, team_id, company:companies(name, year:years(name)), team:teams(name)')
    .order('full_name')
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6">
          <h2 className="font-semibold text-gray-900 mb-4">
            חיילים ({employees?.length ?? 0})
          </h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-right text-gray-500">
                  <th className="pb-2 font-medium">מ&quot;ס</th>
                  <th className="pb-2 font-medium">שם</th>
                  <th className="pb-2 font-medium">אימייל</th>
                  <th className="pb-2 font-medium">שנה</th>
                  <th className="pb-2 font-medium">פלוגה</th>
                  <th className="pb-2 font-medium">צוות</th>
                  <th className="pb-2 font-medium">תפקיד</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {(employees ?? []).map((emp) => {
                  const company = emp.company as unknown as { name: string; year: { name: string } | null } | null
                  const team = emp.team as unknown as { name: string } | null
                  return (
                    <tr key={emp.id}>
                      <td className="py-2 text-gray-600">{emp.military_id}</td>
                      <td className="py-2 text-gray-800 font-medium">{emp.full_name}</td>
                      <td className="py-2 text-gray-600">{emp.email}</td>
                      <td className="py-2 text-gray-600">{company?.year?.name ?? '—'}</td>
                      <td className="py-2 text-gray-600">{company?.name ?? '—'}</td>
                      <td className="py-2 text-gray-600">{team?.name ?? '—'}</td>
